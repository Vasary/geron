...
secret_file="${1:-secrets/open-webui.sops.yaml}"
...
test -f "$secret_output" || {
  printf 'Missing Open WebUI SOPS secret: %s\n' "$secret_rel_to_repo" >&2
  exit 1
}

printf '%s' 'Ollama API key: '
IFS= read -rs ollama_api_key
printf '\n'

if [ -z "$ollama_api_key" ]; then
  printf '%s\n' 'API key must not be empty.' >&2
  exit 1
fi

printf '%s' "$ollama_api_key" | jq -Rs . | SOPS_AGE_KEY_FILE="$age_key_file" sops \
  --config "$sops_config" \
  --filename-override "$secret_rel_to_repo" \
  set \
  --value-stdin \
  "$secret_output" '["data"]["OLLAMA_API_KEY"]'
